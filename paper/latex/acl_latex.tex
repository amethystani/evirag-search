...
\tikzset{
  qbox/.style={rectangle, rounded corners=5pt,
    draw=black!48, fill=gray!7, text width=7.5cm,
  ...
  stdbox/.style={rectangle, rounded corners=4pt,
    draw=orange!68!red, fill=orange!8, text width=3.0cm,
    align=left, font=\small, inner sep=8pt},
  evbox/.style={rectangle, rounded corners=4pt,
    draw=blue!58, fill=blue!7, text width=3.2cm,
    align=left, font=\small, inner sep=8pt},
  sarr/.style={->, line width=1.3pt, color=orange!82!red, >=stealth},
  earr/.style={->, line width=1.3pt, color=blue!68, >=stealth},
}

%%── Shared query box ─────────────────────────────────────────────────────────
\node[qbox] (q) at (4.0, 6.5) {%
  \textbf{Query:}\enspace
  ``Does homework improve academic achievement?''};

%%── Column headers ───────────────────────────────────────────────────────────
\node[hdr, draw=orange!68!red, fill=orange!15, text=orange!82!red]
  (stdh) at (2.0, 5.35) {$r^\mathrm{std}$:\;Standard RAG};
\node[hdr, draw=blue!58, fill=blue!12, text=blue!72]
  (evh)  at (6.3, 5.35) {$r^\mathrm{ev}$:\;\textsc{EVIRAG}};

%%── Query → header arrows ────────────────────────────────────────────────────
\draw[sarr] (q.south) -- ++(0,-0.15) -| (stdh.north);
\draw[earr] (q.south) -- ++(0,-0.15) -| (evh.north);

%%── Standard RAG output box ──────────────────────────────────────────────────
\node[stdbox, below=0.35cm of stdh] (stdbox) {%
  ``Homework \emph{consistently} improves
  performance, stronger for
  grades~7--12.''\\[6pt]
  \rule{2.6cm}{0.3pt}\\[5pt]
  {\scriptsize
  \textcolor{red!62!black}{$\boldsymbol\times$\enspace
  2 of 3 viewpoints erased}\\[2pt]
  VC$_\text{emb}$\,=\,0.33\enspace|\enspace CCS\,=\,0.38}};

%%── EVIRAG output box ────────────────────────────────────────────────────────
\node[evbox, below=0.35cm of evh] (evbox) {%
  \textbf{V1}\enspace\textit{[dominant, 6 src]}\\[1pt]
  {\scriptsize Grade 7--12 gain ($d\!\approx\!0.30$)\\
  \textit{CDA-7: methodological}}\\[5pt]
  \textbf{V2}\enspace\textit{[alt., 3 src]}\\[1pt]
  {\scriptsize No effect controlling for SES\\
  \textit{CDA-7: statistical}}\\[5pt]
  \textbf{V3}\enspace\textit{[minority, 2 src]}\\[1pt]
  {\scriptsize Wellbeing harm, no acad.\ gain\\
  \textit{CDA-7: operational, conf.\ low}}\\[6pt]
  \rule{2.8cm}{0.3pt}\\[5pt]
  {\scriptsize
  \textcolor{blue!72}{$\checkmark$\enspace
  All 3 viewpoints surfaced}\\[2pt]
  VC$_\text{emb}$\,=\,1.00\enspace|\enspace CCS\,=\,0.52}};

%%── Header → box arrows ──────────────────────────────────────────────────────
\draw[sarr] (stdh.south) -- (stdbox.north);
\draw[earr] (evh.south)  -- (evbox.north);

%%── Vertical divider ─────────────────────────────────────────────────────────
\draw[gray!30, densely dashed, line width=0.6pt] (4.15, 5.8) -- (4.15, -0.5);

\end{tikzpicture}}%
\caption{Epistemic collapse vs.\ disagreement-aware generation on a contested
query.
$r^\mathrm{std}$ (Standard RAG): returns one fluent answer, erasing minority
viewpoints and suppressing scientific disagreement.
$r^\mathrm{ev}$ (\system): surfaces three distinct viewpoints with source
counts, CDA-7 causal attribution, and evidence-structured confidence.
VC$_\text{emb}$ and CCS values are on the homework-effectiveness query.}
\label{fig:collapse}
\end{figure}
