...
Each stage addresses a distinct failure mode: adversarial retrieval
supplies minority-view evidence ($-0.162$ CR without it), claim-graph
reasoning structures the resulting contradictions ($-0.138$ CR), and
multi-view synthesis prevents re-collapse at generation ($-0.314$ CR,
...
\system\ Full's CCE$=0.231$ exceeds all baselines (0.108--0.138), which is
expected by design: baselines emit unqualified certainty (or no confidence
signal), scoring low CCE incidentally, while \system\ emits explicit calibrated
uncertainty labels derived from CDA-7 priors and source counts.  When evidence
contains partial dissent, \system\ deliberately under-claims certainty---CCE
penalizes this conservatism.  We treat CCE as a diagnostic rather than an
optimization target; the human evaluation ($\alpha{=}0.72$, $p{<}0.001$)
provides the primary completeness signal.
...
Full per-domain results appear in Appendix~\ref{app:protocol}
(Table~\ref{tab:perdomain}).

\paragraph{Case study: homework effectiveness.}
On the homework query, Vanilla RAG returns one view
(VC$_\text{emb}{=}0.33$; CCS$=0.38$); \system\ Full returns all three with
provenance and CDA-7 attribution (VC$_\text{emb}{=}1.00$; CCS$=0.52$)---the
contrast is illustrated in Figure~\ref{fig:collapse}.
Extended examples appear in Appendix~\ref{app:examples}.
...
\system\ Full scores 4.3/5 versus vanilla RAG's 2.1/5
(Krippendorff's $\alpha{=}0.72$; Mann--Whitney $p{<}0.001$;
Cohen's $d{=}2.71$).
...
Cross-backbone evaluation on the education subset with two additional model
families confirms that \system\ consistently outperforms vanilla RAG across
model scales (Appendix~\ref{app:robustness}).

\paragraph{Error analysis.}
\label{sec:error}
Manual inspection of the 22 low-CR queries identifies three failure
categories (full analysis in Appendix~\ref{app:error}):
\textbf{NLI noise and CDA-7 boundary confusion} (50\%),
\textbf{claim granularity mismatch} (32\%), and
\textbf{retrieval coverage gaps} (18\%).
Future gains are more likely from stronger verifiers and retrieval
breadth than from structural changes to the pipeline.

%% ─────────────────────────────────────────────────────────────────────────────
\section{Conclusion}
\label{sec:conclusion}

Standard scientific RAG fails not from missing evidence but from compressing
contested evidence into a single answer---\emph{epistemic collapse}.
\system\ addresses this by surfacing and structuring disagreement through
adversarial retrieval, claim-graph reasoning, multi-view synthesis, and
calibrated confidence, substantially improving contradiction recall and
viewpoint coverage while remaining competitive on faithfulness.
These results support disagreement preservation as a primary design
objective for contested scientific QA.
