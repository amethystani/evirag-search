...
5-point scale; Krippendorff's $\alpha{=}0.72$).  These results suggest that
treating disagreement as a first-class output target can improve how
scientific RAG systems represent contested evidence in disagreement-heavy QA
settings.
\end{abstract}
...
corpus rather than the model's internal confidence.  The CDA-7 taxonomy is
motivated by philosophy-of-science work on how methodological, population, and
theoretical differences sustain scientific disagreement
...
papers (75 papers total; ${\sim}$3,200 text chunks and
${\sim}$4,800 atomic claims).  Each query is annotated with
(i)~ground-truth viewpoints and provenance, (ii)~contradictory claim pairs with
CDA-7 labels, and (iii)~a controversy class.  Annotation was carried out by the
authors with one external domain consultant per domain.
Additional annotation materials, 25 sample queries, prompt templates, and the
CDA-7 decision tree are available at \url{https://anonymous.4open.science/r/evirag-bench}
...
with Bonferroni correction.  Because VC$_\text{emb}$ uses the same encoder
family as retrieval, we interpret it jointly with VC$_\text{kw}$ and human
evaluation rather than as a fully retrieval-agnostic metric.
...
grounding.  CCE does not improve monotonically: \system\ is more conservative in
its confidence labeling than baselines that emit largely unstructured certainty.
We therefore treat CCE as a diagnostic rather than an optimization target:
it penalizes conservative uncertainty labels even when those labels improve
benchmark-relative epistemic-completeness judgments in the assisted human
study.
...
\paragraph{Benchmark bias control.}
\bench\ is annotated at the level of viewpoints, contradiction pairs, and
controversy classes rather than around any single system output.  External
domain consultants annotated viewpoint structure independently before
...
\paragraph{Agreement reporting.}
Independent consultant annotation was used for viewpoint structure on the
benchmark, while the reliability figures reported below come from a 50-query
re-annotation subset for CDA-7 conflict labeling and epistemic-completeness
rating.
For independently replicated annotation tasks, we report Krippendorff's
$\alpha{=}0.72$ for epistemic-completeness ratings and
$\kappa{=}0.74$ for CDA-7 conflict labeling on the 50-query re-annotation
subset.  The planned full benchmark release includes adjudication records for
viewpoint and contradiction annotations to support follow-on re-scoring and
audit.
