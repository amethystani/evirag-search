...
%%── Styles ────────────────────────────────────────────────────────────────
\tikzset{
  qbox/.style={rectangle, rounded corners=7pt, line width=0.9pt,
    text width=6.2cm, align=left, inner sep=12pt},
  abox/.style={rectangle, rounded corners=8pt,
    draw=orange!60!black, fill=yellow!22, line width=1.3pt,
    text width=4.8cm, align=center, inner sep=13pt},
  lnode/.style={circle, fill=black!82, minimum size=8pt, inner sep=0pt},
  qnode/.style={circle, fill=green!62!black, minimum size=11pt, inner sep=0pt},
  epath/.style={->, line width=1.4pt, color=orange!82!red, >=stealth,
    dash pattern=on 7pt off 3.5pt},
  cpath/.style={->, line width=1.4pt, color=blue!65, >=stealth,
    dash pattern=on 7pt off 3.5pt},
  apath/.style={->, line width=1.1pt, color=black!50, >=stealth,
    dash pattern=on 5pt off 3pt},
  elab/.style={fill=white, inner sep=2.5pt, font=\small, text=black!55},
}

%%── Left box: Standard RAG ────────────────────────────────────────────────
\node[qbox, draw=orange!58!red, fill=orange!6] (qstd) at (0, 2.6) {%
  \textbf{\textcolor{orange!82!red}{$r^\mathrm{std}$\;:\;Standard RAG}}\\[6pt]
  ``Does homework improve\\[2pt]
  \colorbox{orange!30}{\strut\textbf{academic achievement}}?''\\[6pt]
  {\small\textcolor{gray!58}{$\hookrightarrow$ selects dominant view only}}%
};

%%── Left box: EVIRAG ─────────────────────────────────────────────────────
\node[qbox, draw=blue!50, fill=blue!5] (qev) at (0, -2.5) {%
  \textbf{\textcolor{blue!65}{$r^\mathrm{ev}$\;:\;\textsc{EVIRAG}}}\\[6pt]
  ``Does homework affect\\[2pt]
  \colorbox{blue!18}{\strut\textbf{academic achievement}}?''\\[6pt]
  {\small\textcolor{gray!58}{$\hookrightarrow$ preserves all conflicting views}}%
};

%%── Circle background (Evidence Hyperspace) ──────────────────────────────
%% Slightly smaller circle moved left → gives space for v2 label before answer box
\fill[gray!11] (8.8, 0.1) circle (3.1);
\draw[gray!36, line width=0.55pt] (8.8, 0.1) circle (3.1);
\node[font=\small\itshape, text=blue!58] at (10.0, -3.65)
  {Evidence Hyperspace};  %% shifted right to clear v3 label

%%── Graph nodes ──────────────────────────────────────────────────────────
\node[qnode] (qn)  at (6.05, 0.15) {};   %% query entry (green)
\node[lnode] (v1)  at (8.95, 2.85) {};   %% Grade 7-12 gain (top)
\node[lnode] (v2)  at (11.40, 0.40) {};  %% No SES effect (right, moved left)
\node[lnode] (v3)  at (7.85, -2.65) {};  %% Wellbeing harm (bottom)

%%── Slanted floating labels — carefully positioned to avoid overlaps ─────
%% v1 label: top, slanted up-right
\node[font=\small\bfseries, text=orange!80!red, rotate=28]
  at (8.35, 3.60)  {Grade 7--12 gain ($d{\approx}0.30$)};
%% v2 label: moved ABOVE v2, short text, well left of answer box
\node[font=\small\bfseries, text=red!65!black, rotate=10]
  at (12.50, 1.70) {No SES effect};
%% v3 label: bottom-left, clear of "Evidence Hyperspace" text
\node[font=\small\bfseries, text=teal!62!black, rotate=18]
  at (6.15, -3.45) {Wellbeing harm};
\node[font=\footnotesize\bfseries, text=green!55!black, above=3pt of qn]
  {Query};

%%── Edge labels inside circle ────────────────────────────────────────────
\node[elab] at (10.00, 1.85) {Contradicts};
\node[elab] at (10.35, -1.20) {Supersedes};

%%── Standard RAG path (orange): query → V1 only ──────────────────────────
\draw[epath] (qstd.east) to[bend right=10] (qn);
\draw[epath] (qn)        to[bend right=8]  (v1);

%%── EVIRAG path (blue): query → V3 → V2 → V1 ───────────────────────────
\draw[cpath] (qev.east)  to[bend left=10]  (qn);
\draw[cpath] (qn)        to[bend left=8]   (v3);
\draw[cpath] (v3)        to[bend right=15] (v2);
\draw[cpath] (v2)        to[bend right=18] (v1);

%%── Answer box — moved further right to clear node labels ────────────────
\node[abox] (ans) at (17.6, 0.2) {%
  \textbf{\textcolor{orange!75!black}{$a_x$\;:\;Multi-View Answer}}\\[9pt]
  \textbf{View~1:}\;Grade~7--12 gain\;$\checkmark$\\[5pt]
  \textbf{View~2:}\;No~SES~effect\;$\checkmark$\\[5pt]
  \textbf{View~3:}\;Wellbeing~harm\;$\checkmark$%
};

%%── Arrow: v1 → answer box (gentle rightward arc) ────────────────────────
\draw[apath] (v1.east) to[bend right=5] (ans.north west);

%%── Bottom legend ────────────────────────────────────────────────────────
\draw[epath, line width=1.3pt] (3.2, -5.10) -- (5.0, -5.10);
\node[right, font=\small, text=gray!60] at (5.0, -5.10)
  {$r^\mathrm{std}$\;:\;Standard RAG\;(single-view collapse)};
\draw[cpath, line width=1.3pt] (12.5, -5.10) -- (14.3, -5.10);
\node[right, font=\small, text=gray!60] at (14.3, -5.10)
  {$r^\mathrm{ev}$\;:\;\textsc{EVIRAG}\;(multi-view preservation)};

\end{tikzpicture}}%
\caption{The claim graph structure of the Evidence Hyperspace.
$r^\mathrm{std}$: Standard RAG takes a dominant-view path, selecting only the
most-supported claim community and collapsing all other views.
$r^\mathrm{ev}$: \system\ traverses all claim communities---following
contradiction and supersession edges---and preserves each conflicting
viewpoint through to the final multi-view output.}
\label{fig:collapse}
\end{figure*}
